...
begin
  if Boolean(io_len > 1) then
    BX_PANIC(Format('pit: io read from port %04x, len:=%u',[unsigned(address),
             unsigned(io_len)]));
  ...
  case (address) of
    ...
    $61:
      ...
      BX_PANIC(Format('pit: unsupported io read from port %04x',[address]));
    end;
...
{$ifend}  // !BX_USE_PIT_SMF
  ...
  if Boolean(io_len > 1) then
    BX_PANIC(Format('pit: io write to port %04x, len:=%u',[unsigned(address),
             unsigned(io_len)]));
...
{$if 0=1}
BX_INFO(('timer 0-2 mod_e control: comm:%02x mod_e:%02x bcd_mod_e:%u',
  (unsigned) command, (unsigned) mod_e, (unsigned) bcd_mod_e));
{$ifend}

      if Boolean( (mode > 5) or (command > $0e) ) then
        BX_PANIC(Format('pit: outp(43h):=%02xh out of range',[unsigned(value)]));
      if Boolean(bcd_mode) then
        BX_PANIC(Format('pit: outp(43h):=%02xh: bcd mod_e unhandled',[
          unsigned(bcd_mode)]));
      ...
      if (Command=$1) or (Command=$2) then
        ...
        (* timer 0: MSB mod_e *)
         begin
          BX_PANIC(Format('pit: outp(43h): command %02xh unhandled',[
            unsigned(command)]));
         end;
      if Command = $3 then  (* timer 0: 16-bit mod_e *)
         begin
          ...
          if Boolean( ((mode<>3) and (mode<>2) and (mode<>0)) or (bcd_mode<>0) ) then
            BX_PANIC(Format('pit: outp(43h): comm 3, mod_e %02x, bcd %02x unhandled',
       ...
       if (Command = $5) or (Command=$6) then (* timer 1: LSB mod_e *)(* timer 1: MSB mod_e *)
          begin
            BX_INFO(Format('pit: outp(43h): command %02xh unhandled (ignored)',
              [unsigned(command)]));
          end;
       if Command = $7 then (* timer 1: 16-bit mod_e *)
          begin
            ...
            if Boolean( ((mode<>3) and (mode<>2) and (mode<>0)) or (bcd_mode<>0)) then
              BX_PANIC(Format('pit: outp(43h): comm 7, mode %02x, bcd %02x unhandled',
                [unsigned(mode), bcd_mode]));
          end;
       ...
       if (Command = $9) or (Command = $a) then  (* timer 2: LSB mode *)(* timer 2: MSB mode *)
          begin
            BX_PANIC(Format('pit: outp(43h): command %02xh unhandled',[unsigned(command)]));
          end;
       if (Command = $b) then (* timer 2: 16-bit mode *)
          begin
            ...
            if Boolean( ((mode<>3) and (mode<>2) and (mode<>0)) or (bcd_mode<>0)) then
              BX_PANIC(Format('pit: outp(43h): comm Bh, mode %02x, bcd %02x unhandled',
                [unsigned(mode), bcd_mode]));
          end;
{$if 0=1}
...
{$ifend}
        if (Command = $c) or (Command = $d) or (Command = $e) or (Command = $f) then
          begin
            BX_INFO(Format('pit: ignoring 8254 command %u',[unsigned(command)]));
          end;

        if (Command > $f) then (* $c  and $f *)
          begin
            BX_PANIC(Format('pit: outp(43h) command %1xh unhandled',
             [unsigned( command)]));
          end;
        end;
  ...
  {$ifend}
      ...
      BX_PANIC(Format('pit: unsupported io write to port %04x := %02x',
       [unsigned( address),unsigned( value)]));
    end;
end;
...
begin
      ...
      begin
        if Boolean(Self.s.timer[timerid].input_latch_toggle=0) then begin
          ...
          if Boolean(BX_PIT_DEBUG) then
            BX_INFO(Format('pit: Self.s.timer[timerid] write L := %02x',[unsigned( value)]));
          end
        else begin
          ...
          xfer_complete := 1;
          if Boolean(BX_PIT_DEBUG) then
            BX_INFO(Format('pit: Self.s.timer[timerid] write H := %02x',[unsigned( value)]));
          end;
      end;
  ...
  if Boolean(xfer_complete) then begin
    ...
    case (Self.s.timer[timerid].mode) of
      ...
      1:
        begin
          BX_PANIC(Format('pit:write_count_reg(%u): mode1 unsupported',[timerid]));
        end;
      2:
        begin
          if Boolean( Self.s.timer[timerid].counter_max = 1 ) then
            BX_PANIC(Format('pit:write_count_reg(%u): mode %u counter_max:=1',
      ...
      4:
        begin
          BX_PANIC(Format('pit:write_count_reg(%u): mode4 unsupported',[timerid]));
        end;
      5:
        begin
          BX_PANIC(Format('pit:write_count_reg(%u): mode5 unsupported',[timerid]));
        end;
      end;
    end;
end;
...
begin
  ...
  BX_INFO(Format('CV:=%04x',[unsigned( Self.s.timer[timerid].counter)]));
    end;
...
begin
  ...
  if Boolean(Self.s.timer[timerid].output_latch_full) then begin
    BX_INFO(Format('pit: pit(%u) latch: output latch full, ignoring',[timerid]));
    exit;
    end;
  ...
  if Boolean(BX_PIT_DEBUG) then
    begin
      BX_INFO(Format('pit: latch_value := %d',[Self.s.timer[timerid].output_latch_value]));
...
begin
  ...
  if Boolean(value) then begin (* PIT2: GATE transition from 0 to 1 *)
    ...
    case( Self.s.timer[2].mode ) of
      ...
      5:    // default:
        begin
          BX_PANIC(Format('bx_pit_c.set_GATE: unhandled timer2 mode %u', [unsigned( Self.s.timer[2].mode)]));
        end;
      end;
    end
...
begin
  ...
  BX_INFO(Format('timer %d period set to %d hz',[timerid, period_hz]));

  if (Self.s.timer[timerid].mode<>0) and (Self.s.timer[timerid].mode<>2)
    and (Self.s.timer[timerid].mode<>3) then BX_PANIC('PANIC IN pit.start');
end;
      ...
      if Boolean( Bool(Self.s.timer[i].GATE) and Bool(Self.s.timer[i].active) ) then begin
        case ( Self.s.timer[i].mode ) of
          ...
          1: // mode 1: Retriggerable One-Shot
            begin
              // wraps after count expires
              BX_PANIC(Format('bx_pit_c.periodic: bad mode: timer[%u], mode %u',
