...
procedure bx_keyb_c.resetinternals(powerup:Bool);
var
  i:Bit32u;
begin
...
procedure bx_keyb_c.init(d:Pbx_devices_c; cmos:Pbx_cmos_c);
var
  i:Bit32u;
begin
  BX_DEBUG(('Init $Id: keyboard.cc,v 1.53 2002/03/27 16:42:54 bdenney Exp $'));
...
procedure bx_keyb_c.paste_delay_changed;
begin
  self.pastedelay := KEYBOARD_PASTE_DELAY div BX_IODEV_HANDLER_PERIOD;
  BX_INFO(Format('will paste characters every %d keyboard ticks',[self.pastedelay]));
end;
...
//
function bx_keyb_c.read_handler(this_ptr:pointer; address:Bit32u; io_len:unsigned):Bit32u;
var
  val_:Bit8u;
  i:unsigned;
begin
  if (io_len > 1) then
    BX_PANIC(Format('kbd: io read to address %08x, len:=%u',[unsigned(address),
      unsigned(io_len)]));


bx_debug(Format( 'read from port $%04x', [address]));

  if (address = $60) then begin (* output buffer *)
    if (self.s.kbd_controller.auxb)<>0 then begin (* mouse byte available *)
      ...
      if (self.s.controller_Qsize)<>0 then begin
...
//bx_debug(('mouse: ___io_read aux := $%02x', (unsigned) val_));
  ...
  else if (self.s.kbd_controller.outb)<>0 then begin (* kbd byte available *)
...
bx_debug(( '___io_read kbd'));
      ...
      if (self.s.controller_Qsize)<>0 then begin
        ...
        i:=0;
        while i<self.s.controller_Qsize-1 do begin
          // move Q elements towards head of queue by one
          self.s.controller_Q[i] := self.s.controller_Q[i+1];
          Inc(i);
          end;
...
{$if BX_CPU_LEVEL >= 2}
  else if (address = $64) then begin (* status register *)
...
{$else} (* BX_CPU_LEVEL > 0 *)
  (* XT modE, System 8255 mode Register *)
    ...
    return self.s.kbd_controller.outb;
    end;
{$ifend} (* BX_CPU_LEVEL > 0 *)

  BX_PANIC(Format('unknown address in io read to keyboard port %x',[unsigned(address)]));
  Result:=0; (* keep compiler happy *)
end;
  // static IO port write callback handler
  // redirects to non-static class handler to aprocedure virtual functions

procedure bx_keyb_c.write_handler(this_ptr:Pointer; address:Bit32u; value:Bit32u; io_len:unsigned);
var
  command_byte:Bit8u;
...
{$ifend}  // !BX_USE_KEY_SMF
  ...
  if (io_len > 1) then
    BX_PANIC(Format('kbd: io write to address %08x, len:=%u',[
  ...
  case address of
    $60:
      ...
      // if expecting data byte from command last sent to port 64h
        if (self.s.kbd_controller.expecting_port60h)<>0 then begin
          self.s.kbd_controller.expecting_port60h := 0;
          // data byte written last to $60
          self.s.kbd_controller.c_d := 0;
          if (self.s.kbd_controller.inpb)<>0 then begin
        ...
        case (self.s.kbd_controller.last_comm) of
          $60: // write command byte
            begin
              ...
              if ((self.s.kbd_controller.allow_irq12<>0) and (self.s.kbd_controller.auxb<>0)) then
                self.s.kbd_controller.irq12_requested := 1
              else if ((self.s.kbd_controller.allow_irq1 <> 0) and (self.s.kbd_controller.outb <> 0)) then
                self.s.kbd_controller.irq1_requested := 1;
          ...
          $d3: // write mouse output buffer
            ...
            BX_PANIC(Format('=:= unsupported write to port 60h(lastcomm:=%02x): %02x',[
        ...
        if (self.s.kbd_controller.kbd_clock_enabled=0) then begin
          BX_ERROR(Format('keyboard disabled  and send of byte %02x to kbd',[unsigned(value)]));
          end;
    ...
    $64: // control register
      begin
        ...
        case value of
          $20: // get keyboard command byte
            begin
              ...
              if (self.s.kbd_controller.outb)<>0 then begin
          			BX_ERROR(Format('kbd: OUTB set and command $%02x encountered',[value]));
        ...
        $60: // write command byte
          ...
          // following byte written to port 60h is command byte
            self.s.kbd_controller.expecting_port60h := 1;
          end;

        $a1:
          begin
            BX_ERROR(('Dummy out Green PC for now : $a1'));
          end;

        $a7: // disable the aux device
          begin
            set_aux_clock_enable(0);
            BX_DEBUG(('aux device disabled'));
          end;
        $a8: // enable the aux device
          begin
        ...
        $a9: // Test Mouse Port
          begin
            // controller output buffer must be empty
            if (self.s.kbd_controller.outb)<>0 then begin
		        	BX_PANIC(Format('kbd: OUTB set and command $%02x encountered',[value]));
              Halt; // Vai alla fine del case
            end;
	  ...
	  // (mch) Why is self commented out??? Enabling
        ...
        $d1: // write output port: next byte written to port 60h
          begin
        ...
        $d3: // write mouse output buffer
          begin
        	  //FIXME: Why was self a panic?
            BX_DEBUG(('io write $64: command := $D3(write mouse outb)'));
        ...
        $d4: // write to mouse
          begin
            BX_DEBUG(('io write $64: command := $D4 (write to mouse)'));
        ...
        $e0: // Read Test Inputs
          begin
            BX_PANIC(Format('io write $64: command := %02xh', [unsigned(value)]));
          end;
        ...
        $fe: // System Reset, transition to real mode
          ...
          begin
            if ((value=$ff) or ((value>=$f0) and (value<=$fd))) then begin
              (* useless pulse output bit commands ??? *)
              BX_DEBUG(Format('[KBD]io write to port 64h, useless command %02x',[unsigned(value)]));
              exit;
            end;
            BX_PANIC(Format('unsupported io write to keyboard port %x, value := %x',[unsigned(address),
              unsigned(value)]));
          end;
        end;
    end;
...
begin
  ...
  while (self.pastebuf_ptr < self.pastebuf_len) do begin
    ...
    if (entry=nil) then begin
      BX_ERROR (Format('paste character $%02x ignored',[byte]));
    end else begin
...
procedure bx_keyb_c.paste_bytes (bytes:PBit8u; length:Bit32s);
begin
  BX_DEBUG (Format('paste_bytes: %d bytes',[length]));
  if (self.pastebuf)<>nil then begin
    BX_ERROR (Format('previous paste was not completed!  %d chars lost',[self.pastebuf_len - self.pastebuf_ptr]));
  end;
...
procedure bx_keyb_c.gen_scancode(Key:Bit32u);
var
  escaped:Bit8u;
  scancode:PChar;
  i:Bit8u;
...
procedure bx_keyb_c.set_kbd_clock_enable(value:Bit8u);
var
  prev_kbd_clock_enabled:Bool;
begin
  if (value=0) then begin
    self.s.kbd_controller.kbd_clock_enabled := 0;
    end
  else begin
    ...
    if ((prev_kbd_clock_enabled=0) and (self.s.kbd_controller.outb=0)) then begin
      activate_timer;
      end;
    end;
end;

procedure bx_keyb_c.set_aux_clock_enable(value:Bit8u);
var
...
function bx_keyb_c.get_kbd_enable:Bit8u;
begin
...
procedure bx_keyb_c.controller_enQ(data:Bit8u; source:unsigned);
begin
  ...
  // the Q is empty
  if (source = 0) then begin // keyboard
...
procedure bx_keyb_c.kbd_enQ_imm(val:Bit8u);
var
  tail:Integer;
begin

      if (self.s.kbd_internal_buffer.num_elements >= BX_KBD_ELEMENTS) then begin
...
procedure bx_keyb_c.kbd_enQ(scancode:Bit8u);
var
  tail:Integer;
begin
  ...
  if (self.s.kbd_internal_buffer.num_elements >= BX_KBD_ELEMENTS) then begin
    BX_INFO(Format('internal keyboard buffer full, ignoring scancode.(%02x)',[scancode]));
    exit;
  end;
  ...
  if ((self.s.kbd_controller.outb=0) and (self.s.kbd_controller.kbd_clock_enabled<>0)) then begin
    activate_timer;
...
function bx_keyb_c.mouse_enQ_packet(b1:Bit8u;b2:Bit8u;b3:Bit8u):Bool;
begin
  if ((self.s.mouse_internal_buffer.num_elements + 3) >= BX_MOUSE_BUFF_SIZE) then begin
    Result:=0; (* buffer doesn't have the space *)
    Exit;
    end;

BX_DEBUG(Format('[KBD]mouse: enQ_packet(%02x, %02x, %02x)',[b1,b2,b3]));
...
procedure bx_keyb_c.mouse_enQ(mouse_data:Bit8u);
var
  tail:Integer;
begin
  ...
  if (self.s.mouse_internal_buffer.num_elements >= BX_MOUSE_BUFF_SIZE) then begin
    BX_ERROR(Format('mouse: internal mouse buffer full, ignoring mouse data.(%02x)',[mouse_data]));
    exit;
    end;
bx_debug(Format( '# mouse_enq() aux_clock_enabled := %u',[self.s.kbd_controller.aux_clock_enabled]));
  ...
  if ((self.s.kbd_controller.outb=0) and (self.s.kbd_controller.aux_clock_enabled<>0)) then begin
    activate_timer;
bx_debug(( '# activating timer...'));
    exit;
    end;
...
procedure bx_keyb_c.kbd_ctrl_to_kbd(value:Bit8u);
var
  cps:double;
begin
  ...
  if (self.s.kbd_internal_buffer.expecting_typematic)<>0 then begin
    self.s.kbd_internal_buffer.expecting_typematic := 0;
    self.s.kbd_internal_buffer.delay := (value shr 5)  and $03;
    case (self.s.kbd_internal_buffer.delay) of
      0: BX_INFO(('setting delay to 250 mS (unused)'));
      1: BX_INFO(('setting delay to 500 mS (unused)'));
      2: BX_INFO(('setting delay to 750 mS (unused)'));
      3: BX_INFO(('setting delay to 1000 mS (unused)'));
    end;
    self.s.kbd_internal_buffer.repeat_rate := value  and $1f;
    cps := 1 /((8 + (value  and $07)) * exp(log10(2) * ((value shr 3)  and $03)) * 0.00417);
    BX_INFO(Format('setting repeat rate to %.1f cps (unused)',[cps]));
    kbd_enQ($FA); // send ACK
    exit;
    end;

  if (self.s.kbd_internal_buffer.expecting_led_write)<>0 then begin
    self.s.kbd_internal_buffer.expecting_led_write := 0;
  ...
  if (self.s.kbd_controller.expecting_scancodes_set)<>0 then begin
    self.s.kbd_controller.expecting_scancodes_set := 0;
    if( value <> 0 ) then begin
      if( value<4 ) then begin
        self.s.kbd_controller.current_scancodes_set := (value-1);
        BX_INFO(Format('Switched to scancode set %d\n',[self.s.kbd_controller.current_scancodes_set]));
        kbd_enQ($FA);
        end
      else begin
        BX_ERROR(Format('Received scancodes set out of range: %d\n',[value]));
...
(*      if (bx_options.Okeyboard_type^.get() <> BX_KBD_XT_TYPE) then begin
    ...
    $d3:
      ...
      kbd_enQ($fa);
			 ...
			 *)
       begin
        BX_ERROR(Format('kbd_ctrl_to_kbd(): got value of %02x',[value]));
        kbd_enQ($FA); (* send ACK ??? *)
        exit;
      end;
    end;
...
function  bx_keyb_c.periodic( usec_delta:Bit32u ):unsigned;
var
  retval:Bit8u;
begin
  ...
  if (self.s.kbd_controller.kbd_clock_enabled )<>0 then begin
    ...
    if(count_before_paste>=self.pastedelay) then begin //!!!!!!!!!!!!!!
  ...
  if ( self.s.kbd_controller.timer_pending = 0 ) then begin
    Result:=retval;
    Exit;
    end;

  if ( usec_delta >= self.s.kbd_controller.timer_pending ) then begin
    self.s.kbd_controller.timer_pending := 0;
    end
  else begin
  ...
  if ((self.s.kbd_controller.kbd_clock_enabled<>0) and (self.s.kbd_internal_buffer.num_elements<>0)) then begin
  bx_debug( '#   servicing keyboard code');
    ...
    if ((self.s.kbd_controller.aux_clock_enabled<>0) and (self.s.mouse_internal_buffer.num_elements<>0)) then begin
...
procedure bx_keyb_c.activate_timer;
begin
  if (self.s.kbd_controller.timer_pending = 0) then begin
      self.s.kbd_controller.timer_pending := KEYBOARD_DELAY;
  end;
end;

function bx_keyb_c.get_resolution_byte:Bit8u;
var
  ret:Bit8u;
begin
...
function bx_keyb_c.get_status_byte:Bit8u;
var
  ret:Bit8u;
begin
  if (s.mouse.mode = MOUSE_MODE_REMOTE) then ret:=$40 else ret:=0;
  ret := ret or  (s.mouse.enable shl 5);
  if s.mouse.scaling=1 then ret:= ret or 0 else ret:=ret or 1 shl 4;
  ret := ret or ((s.mouse.button_status and $1) shl 2);
  ret := ret or ((s.mouse.button_status and $2) shl 0);
  Result:=ret;
end;

procedure bx_keyb_c.kbd_ctrl_to_mouse(value:Bit8u);
begin
 ...
 if (self.s.kbd_controller.expecting_mouse_parameter)<>0 then begin
       ...
       case (self.s.kbd_controller.last_mouse_command) of
         ...
         begin
    		   case (value) of
      			 0:
               begin
        	       self.s.mouse.resolution_cpmm := 1;
			         end;
      			 1:
               begin
  	  		       self.s.mouse.resolution_cpmm := 2;
	  		       end;
			       2:
               begin
		    	       self.s.mouse.resolution_cpmm := 4;
			         end;
      			 3:
               begin
			           self.s.mouse.resolution_cpmm := 8;
			         end;
			       else
               begin
			           BX_PANIC(Format('[mouse] Unknown resolution %d', [value]));
               end;
   		   end;
		   ...
		   controller_enQ($FA, 1); // ack
  		   ...
  		   BX_PANIC(Format('MOUSE: unknown last command (%02xh)',[self.s.kbd_controller.last_mouse_command]));
       end;
 end else begin
  self.s.kbd_controller.expecting_mouse_parameter := 0;
  ...
  if (self.s.mouse.mode = MOUSE_modE_WRAP) then begin
    // if not a reset command or reset wrap mode
    // then just echo the byte.
    if ((value <> $ff) and (value <> $ec)) then begin
      if (True) then
      	BX_INFO(Format('[mouse] wrap mode: Ignoring command %$02.',[value]));
      controller_enQ(value,1);
      // bail out
      exit;
    end;
  ...
  case ( value ) of
    ...
    $eb: // Read Data (send a packet when in Remote mode)
      ...
      //FEh Resend
      BX_PANIC(Format('MOUSE: kbd_ctrl_to_mouse(%02xh)',[value]));
    end;
 end;
end;

procedure bx_keyb_c.create_mouse_packet(force_enq:bool);
var
  b1, b2, b3:Bit8u;
  delta_x:Bit16s;
  delta_y:Bit16s;
...
begin
  ...
  if((force_enq=0) and (delta_x=0) and (delta_y=0)) then begin
    exit;
  end;
  ...
  if ( (delta_x>=0) and (delta_x<=255) ) then begin
    b2 := delta_x;
    dec(self.s.mouse.delayed_dx,Delta_x);
    end
  else if ( delta_x > 255 ) then begin
    b2 := $ff;
    dec(self.s.mouse.delayed_dx,255);
    end
  else if ( delta_x >= -256 ) then begin
    b2 := delta_x;
    b1 := b1 or $10;
    dec(self.s.mouse.delayed_dx,delta_x);
    end
  ...
  if ( (delta_y>=0) and (delta_y<=255) ) then begin
    b3 := delta_y;
    dec(self.s.mouse.delayed_dy,delta_y);
    end
  else if ( delta_y > 255 ) then begin
    b3 := $ff;
    dec(self.s.mouse.delayed_dy,255);
    end
  else if ( delta_y >= -256 ) then begin
    b3 := delta_y;
    b1 := b1 or $20;
    dec(self.s.mouse.delayed_dy,delta_y);
    end
...
procedure bx_keyb_c.mouse_enabled_changed(enabled:bool);
begin
  if((self.s.mouse.delayed_dx<>0) or (self.s.mouse.delayed_dy<>0)) then begin
...
procedure bx_keyb_c.mouse_motion(delta_x:Integer; delta_y:Integer;button_state:unsigned);
var
  force_enq:bool;
begin
  force_enq:=0;
...
{$if VERBOSE_KBD_DEBUG=1}
  if (delta_x !:= 0 or delta_y !:= 0)
    BX_DEBUG(('[mouse] Dx:=%d Dy:=%d', delta_x, delta_y));
{$ifend}  (* ifdef VERBOSE_KBD_DEBUG *)

  if( (delta_x=0) and (delta_y=0) and (self.s.mouse.button_status = button_state  and $3) ) then begin
    BX_DEBUG(('Ignoring useless mouse_motion call:\n'));
    BX_DEBUG(('self should be fixed in the gui code.\n'));
    exit;
  end;

  if(self.s.mouse.button_status <> (button_state  and $3)) then begin
    force_enq:=1;
  end;
  ...
  if((self.s.mouse.delayed_dx>255)or
     ...
     (self.s.mouse.delayed_dy<-256)) then begin
    force_enq:=1;
  end;
...
procedure bx_keyb_c.put_scancode( code:PChar; count:Integer  );
var
  I:Integer;
begin
  I:=0;
