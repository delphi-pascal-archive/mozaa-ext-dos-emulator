...
constructor bx_cmos_c.Create;
begin
end;

destructor bx_cmos_c.Destroy;
begin
  inherited;
end;
...
{$ifend}
	...
	if (Options_cmos_time = 1) then
		   self.s.timeval := time
	else if (Options_cmos_time <> 0) then
		   self.s.timeval := 0;
	...
	BX_INFO(Format('Setting initial clock to: %d  ---  occhio!!!',[0]));

	update_clock();

	// load CMOS from image file if requested.
  if Boolean(Options_cmos_use_image) then begin(*
...
begin
	...
	bx_pc_system.activate_timer(self.s.one_second_timer_index,
			                   1000000, 1);
...
begin
    ...
    if ( self.s.reg[$0b] and $40 )<>0 then
      bx_pc_system.activate_timer(self.s.periodic_timer_index,
...
function bx_cmos_c.read_handler(self_ptr:Pointer; address:Bit32u; io_len:unsigned):Bit32u;
var
	ret8:Bit8u;
begin
	if (io_len > 1) then
    BX_PANIC(Format('io read from address %08x len:=%u',[unsigned(address), unsigned(io_len)]));

	if (BX_CMOS_DEBUG)<>0 then
    BX_INFO(Format('CMOS read of CMOS register $%x',[unsigned(self.s.cmos_mem_address)]));


	case (address) of
    $0071:
      begin
        if (self.s.cmos_mem_address >= BX_NUM_CMOS_REGS) then begin
    	  	 BX_PANIC(Format('unsupported cmos io read, register($%02x)!',[unsigned(self.s.cmos_mem_address)]));
		    end;

      ret8 := self.s.reg[self.s.cmos_mem_address];
      // all bits of Register C are cleared after a read occurs.
      if (self.s.cmos_mem_address = $0c) then begin
        self.s.reg[$0c] := $00;
      ...
      begin
        BX_PANIC(Format('unsupported cmos read, address:=%x!',[unsigned(address)]));
        Result:=0;
        exit;
        end;
    end;
...
{$ifend}  // !BX_USE_CMOS_SMF
	...
	if (io_len > 1) then
    BX_PANIC(Format('io write to address %08x len:=%u', [unsigned(address), unsigned(io_len)]));

	if (BX_CMOS_DEBUG)<>0 then
    BX_INFO(Format('CMOS write to address: $%x := $%x', [unsigned(address), unsigned(value)]));
    ...
    $0071:
      begin
        if (self.s.cmos_mem_address >= BX_NUM_CMOS_REGS) then begin
	      	 BX_PANIC(Format('unsupported cmos io write, register($%02x):=%02x!',[unsigned(self.s.cmos_mem_address), unsigned(value)]));
		     exit;
		    end;
       if self.s.cmos_mem_address in [$00..$09] then begin
       (*case $00: // seconds
       ...
       case $09: *)// year
         ...
         exit;
		   ...
		   if self.s.cmos_mem_address = $0a then begin  // Control Register A
         ...
         if (dcc <> $02) then begin
           BX_PANIC(Format('CRA: divider chain control $%x',[dcc]));
         end;
		 ...
		 if self.s.cmos_mem_address = $0b then begin// Control Register B
		   ...
		   if (value  and $02)=0 then
	  		 BX_PANIC(('write status reg B, 12 hour mod_e enabled.'));
		   ...
		   self.s.reg[$0b] := value;
		   if ( (prev_CRB  and $40) <> (value  and $40) ) then begin
			 // Periodic Interrupt Enabled changed
			 if (prev_CRB  and $40)<>0 then begin
			   ...
			   if ( (self.s.reg[$0a]  and $0f) <> 0 ) then begin
      ...
      begin
		   BX_ERROR(Format('write to control register $%x (read-only)',[self.s.cmos_mem_address]));
       goto endcase;
		  end;
...
begin
  ...
  if (self.s.reg[$0b]  and $40)<>0 then begin
...
begin
  ...
  // update status register C
  if (self.s.reg[$0b]  and $10)<>0 then begin
