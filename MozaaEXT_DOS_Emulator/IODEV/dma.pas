...
type
  ...
  public
    ...
    procedure raise_HLDA(pc_sys:pbx_pc_system_c);
...
begin
  if Boolean(io_len > 1) then begin
    BX_ERROR(Format('io read from address %08x, len:=%u',[address, io_len]));
  	Result:=$ff; exit;
    end;
...
{$ifend}
  ...
  case address of
    ...
    $cc: (* DMA-2 current address, channel 3 *)
      ...
      if Boolean(Self.s[ma_sl].flip_flop=0) then begin
    ...
    $da: // DMA-2: temporary register
      ...
      BX_ERROR(Format('DMA-%d: read of temporary register',[ ma_sl+1]));
    ...
    $008e:
      ...
      begin
        BX_ERROR(Format('read: unsupported address:=%04x',[address]));
        Result:=(0);
        exit;
      end;
    end;
...
{$ifend}  // !BX_USE_DMA_SMF
  ...
  if Boolean(io_len > 1) then begin
    if Boolean( (io_len = 2) and (address = $0b) ) then begin
{$if BX_USE_DMA_SMF=1}
...
{$ifend}
    ...
    BX_ERROR(Format('io write to address %08x, len:=%u',[address, io_len]));
	exit;
    end;
  ...
  case address of
    ...
    $d0: (* DMA-2: command register *)
      begin
        ...
        if Boolean(value <> $04) then
          BX_ERROR(Format('DMA: write to $%02x: value(%02xh) not 04h',[address, value]));
    ...
    $d2: // DMA-2: request register
      begin
        ...
        BX_ERROR(Format('DMA-%d: write to request register (%02x)',[ma_sl+1,value]));
    ...
    $008E:
      ...
      BX_ERROR(Format('write ignored: %04xh := %02xh',[address, value]));
    end;
end;
...
begin
  ...
  if Boolean( ( channel <> 2 ) and ( channel <> 4 ) ) then
    BX_PANIC(Format('DRQ(): channel %d !:= 2 (floppy) or 4 (cascade)',[channel]));
  ...
  if Boolean( (Self.s[ma_sl].chan[channel].mode.mode_type <> DMA_MODE_SINGLE) and
       ...
       (Self.s[ma_sl].chan[channel].mode.mode_type <> DMA_MODE_CASCADE) ) then
        BX_PANIC(Format('DRQ: mod_e_type(%02x) not handled',[Self.s[ma_sl].chan[channel].mode.mode_type]));
  ...
  if Boolean( (dma_base  and ($7fff0000 shl ma_sl)) <> (dma_roof  and ($7fff0000 shl ma_sl)) ) then begin
    BX_INFO(Format('dma_base := %08x', [dma_base]));
    BX_INFO(Format('dma_base_count := %08x',[Self.s[ma_sl].chan[channel].base_count]));
    BX_INFO(Format('dma_roof := %08x',[dma_roof]));
    BX_PANIC(Format('request outside %dk boundary',[64 shl ma_sl]));
  end;
...
procedure bx_dma_c.raise_HLDA(pc_sys:pbx_pc_system_c);
var
  channel:unsigned;
  phy_addr:Bit32u;
  count_expired:Bool;
