...
{$ifend}
    ...
    $0401:
      begin
        ...
        BX_PANIC((Self.s.bios_message));
        end;
      end;
    $0400:
      begin
        BX_PANIC(Format('BIOS panic at rombios.c, line %d', [value]));
      end;
    ...
    $fff0:
      begin
        ...
        if Boolean( Self.s.bios_message_i >= BX_BIOS_MESSAGE_SIZE ) then begin
          ...
          Self.s.bios_message_i := 0;
          BX_INFO(Format('%s', [Self.s.bios_message]));
          end
  ...
  if Boolean(dbgunsupported_io) then
    begin
      case (io_len) of
      1:
        BX_INFO(Format('unmapped: 8-bit write to %04x := %02x', [address, value]));
      2:
        BX_INFO(Format('unmapped: 16-bit write to %04x := %04x',[address, value]));
      4:
        BX_INFO(Format('unmapped: 32-bit write to %04x := %08x',[address, value]));
      else
        BX_INFO(Format('unmapped: ??-bit write to %04x := %x',[address, value]));
      end;
    end;  
end;
end.
