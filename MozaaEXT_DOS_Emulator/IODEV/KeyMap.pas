...
begin
  ...
  if lineptr^ = #0 then
    begin
    Result := -1;  // nothing but spaces until end of line
    Exit;
    end;
  if lineptr^ = #35 then
    begin
    Result := -1;  // nothing but a comment
    Exit;
    end;
...
begin
  ...
  while (True) do
    ...
    if ((buf[0] = #92) and (buf[2] = #92) and (buf[3] = #0)) then
      begin
      ascii^ := Bit32s(buf[1]);
      end
    else
      begin
      if (strcomp(buf, 'space') = 0) then
        begin
        ascii^ := 32;
        end
      else
        begin
        if (strcomp(buf, 'return') = 0) then
          begin
          ascii^ := 13;
          end
        else
          begin
          if (strcomp(buf, 'tab') = 0) then
            begin
            ascii^ := 9;
            end
          else
            begin
            if (strcomp(buf, 'backslash') = 0) then
              begin
              ascii^ := 92;
              end
            else
              begin
              if (strcomp(buf, 'apostrophe') = 0) then
                begin
                ascii^ := 39;
                end
              else
                begin
                if (strcomp(buf, 'none') = 0) then
                  begin
                  ascii^ := -1;
                  end
                else
...
begin
  ...
  BX_INFO(Format('Loading keymap from %s', [filename]));
  init_parse;
  ...
  while (True) do
    ...
    if boolean(strlcomp('XK_', xwinSym, 3) <> 0) then
      begin
      BX_PANIC(Format('keymap line %d: X windows symbol %s must start with XK_',
        [lineCount, xwinSym]));
      end;
    ...
    if (baseKey = BX_KEYMAP_UNKNOWN) then
      begin
      BX_PANIC(Format('line %d: unknown BX_KEY constant %s', [lineCount, baseSym]));
      continue;
      end;

    if (xwinKey = BX_KEYMAP_UNKNOWN) then
      begin
      BX_PANIC(Format('line %d: unknown GUI constant %s', [lineCount, xwinSym]));
      continue;
      end;
  ...
  BX_INFO(Format('Loaded %d symbols', [keymapCount]));
