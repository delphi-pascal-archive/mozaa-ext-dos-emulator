...
uses cpu, Config, Service, SysUtils, Memory;

const
  BX_IODEV_HANDLER_PERIOD = 100;   // microseconds
...
uses
  PIC, Unmapped, cmos, dma, {$if BX_USE_NEW_PIT=1} pit_wrap, pit82c54 {$else} pit {$ifend}
  , keyboard,  vga, floppy,HDD;
...
{$if BX_USE_NEW_PIT=0}
  if ( bx_pit.periodic( BX_IODEV_HANDLER_PERIOD )<>0 ) then
    begin
...
{$ifend}
  ...
  bx_floppy := bx_floppy_ctrl_c.Create;
  bx_floppy.init(@Self, @bx_cmos);
  bx_floppy.reset(BX_RESET_HARDWARE);

  bx_hard_drive:=bx_hard_drive_c.Create;
  bx_hard_drive.init(@Self,@bx_cmos);

  bx_keyboard:=bx_keyb_c.Create;
...
begin
  if Boolean(irq >= BX_MAX_IRQS) then begin
    BX_PANIC(Format('IO device %s registered with IRQ:=%d above %u',[name, irq, BX_MAX_IRQS-1]));
    end;
  if Boolean(irq_handler_name[irq]) then begin
    BX_PANIC(Format('IRQ %u conflict, %s with %s',[ irq, irq_handler_name[irq], name]));
    end;
...
begin
  if Boolean(irq >= BX_MAX_IRQS) then begin
    BX_PANIC(Format('IO device %s tried to unregister IRQ %d above %u',[name, irq, BX_MAX_IRQS-1]));
    end;

  if Boolean(irq_handler_name[irq]=nil) then begin
    BX_INFO(Format('IO device %s tried to unregister IRQ %d, not registered',[name, irq]));
    exit;
  end;

  if Boolean(StrComp(irq_handler_name[irq], name)) then begin
    BX_INFO(Format('IRQ %u not registered to %s but to %s',[irq, name, irq_handler_name[irq]]));
    exit;
    end;
...
begin
  ...
  ret:=bx_read_handler_t(io_read_handler[handle].funct)(io_read_handler[handle].this_ptr, Bit32u(addr), io_len);
