...
uses classes,Config, iodev, cmos,dialogs;
...
begin
  Result:=EXTRACT_FIELD(bx_hard_drive.s[0].controller.buffer,b,s,n);
end;

function get_packet_byte(b:byte):uint8;
begin
  Result:=bx_hard_drive.s[0].controller.buffer[b];
end;

function get_packet_word(b:byte):uint16;
begin
  Result:=(bx_hard_drive.s[0].controller.buffer[b] shl 8) or (bx_hard_drive.s[0].controller.buffer[b+1]);
end;
...
{$ifend}
  ...
  if Boolean(BX_CDROM_PRESENT) then
    ...
    {$if LOWLEVEL_CDROM=1}
      self.s[1].cdrom.cd := new LOWLEVEL_CDROM(bx_options.cdromd.Opath^.getptr ());
    {$ifend}

    {$if LOWLEVEL_CDROM=1}
      if Boolean(bx_options.cdromd.Oinserted^.get () = BX_INSERTED) then begin
            if Boolean(self.s[1].cdrom.cd^.insert_cdrom()) then begin
...
{    if Boolean(bx_options.diskd.Opresent^.get () @ and !bx_options.cdromd.Opresent^.get ()) then begin
  ...
  else if Boolean( BX_BOOT_DRIVE = 2) then begin
	    ...
	    $a0: begin
		  ...
		  if Boolean(index >= 2048) then begin
			...
			case (s[self.drive_select].atapi.command) of
			      ...
			      $a8: // read (12)
      {$if LOWLEVEL_CDROM=1}
				    s[self.drive_select].cdrom.cd^.read_block(bx_hard_drive.s[0].controller.buffer,
									s[self.drive_select].cdrom.next_lba);
				    s[self.drive_select].cdrom.next_lba++;
				    ...
				    s[self.drive_select].atapi.drq_bytes -:= 2048;
...
{$ifend}
              ...
              begin
			      ...
			      $43: begin // read toc
				    if Boolean(s[self.drive_select].cdrom.ready) then begin
{$if LOWLEVEL_CDROM=1}
					  bool msf := (bx_hard_drive.s[0].controller.buffer[1] shr 1)  and 1;
					  uint8 starting_track := bx_hard_drive.s[0].controller.buffer[6];
{$ifend}
					  alloc_length := read_16bit(@bx_hard_drive.s[0].controller.buffer[7]);
...
{$if LOWLEVEL_CDROM=1}
						      ...
						      if Boolean(!s[self.drive_select].cdrom.cd^.read_toc(bx_hard_drive.s[0].controller.buffer,
										       @toc_length, msf, starting_track)) then begin
							    atapi_cmd_error(SENSE_ILLEGAL_REQUEST,
									    ASC_INV_FIELD_IN_CMD_PACKET);
							    raise_interrupt();
						      end; else begin
...
begin
  ...
  while i < 10 do
    begin
    	s[self.drive_select].id_drive[10+i] := (ord(serial_number[i*2]) shl 8) or
	        ord(serial_number[i*2 + 1]);
      inc(i);
    end;
  i:=20;
  while i<=22 do
  ...
  while i < Length(firmware) div 2 do
    begin
    	s[self.drive_select].id_drive[23+i] := (ord(firmware[i*2]) shl 8) or
	      ord(firmware[i*2 + 1]);
      inc(i);
    end;
  assert((23+i) = 27);
  i:=0;
  while i < strlen(model_no) div 2 do begin
    begin
    	s[self.drive_select].id_drive[27+i] := (ord(model_no[i*2]) shl 8) or
	      ord((model_no[i*2 + 1]));
    inc(i);
  end;
  assert((27+i) = 47);
...
{$ifend}
    ...
    // insert cdrom
{$if LOWLEVEL_CDROM=1}
    if Boolean(self.s[1].cdrom.cd^.insert_cdrom(bx_options.cdromd.Opath^.getptr())) then begin
      BX_INFO(( 'Media present in CD-ROM drive'));
      self.s[1].cdrom.ready := 1;
      self.s[1].cdrom.capacity := self.s[1].cdrom.cd^.capacity();
...
var diskfile:TFileStream;
begin
if not fileexists(pathname) then begin
diskfile:=TFileStream.Create(pathname,fmcreate);
diskfile.Size:=HDD_NUM_CYL*HDD_SECTOR_PER_TRACK*HDD_NUM_HEADS*512;
diskfile.Free;
end;
fd:=FileOpen(pathname,fmOpenReadWrite);
  (* look at size of image file to calculate disk geometry *)
if fd<>-1 then  Result:=1
else showmessage('Невозможно открыть файл-снимок НЖМД!');
end;
...
var
   Readed:Integer;
begin
  Result:=FileRead(fd, buf, count);
end;

function default_image_t.write (buf:array_buffer_disk; count:size_t):Bit32u;
begin
if HDD_READ_ONLY = 0 then
begin
  Result:=  filewrite(fd, buf, count);
  end
else
  Result:=count;
end;
