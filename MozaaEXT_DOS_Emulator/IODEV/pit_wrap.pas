...
procedure
bx_pit_c.timer_handler(this_ptr:Pointer);
var
   class_ptr:pbx_pit_c;
begin
...
begin
  ...
  if Boolean((time_passed<>0) or (Self.s.last_next_event_time <> Self.s.timer.get_next_event_time())) then begin
    BX_DEBUG(('pit: RESETting timer.'));
    bx_pc_system.deactivate_timer(Self.s.timer_handle[0]);
    BX_DEBUG(('deactivated timer.'));
    if Self.s.timer.get_next_event_time()<>0 then begin
      bx_pc_system.activate_timer(Self.s.timer_handle[0], //!!!
				  MAX(1,(Self.s.timer.get_next_event_time()*USEC_PER_SECOND) div TICKS_PER_SECOND),
				  0);
      BX_DEBUG(('activated timer.'));
...
begin
  ...
  if Boolean(io_len > 1) then
    BX_PANIC(Format('pit: io read from port %04x, len:=%u',[address,
             io_len]));

  if Boolean(BX_PIT_DEBUG) then
    BX_INFO(Format('pit: io read from port %04x',[address]));
...
{$ifend}  // !BX_USE_PIT_SMF
  ...
  if Boolean(io_len > 1) then
    BX_PANIC(Format('pit: io write to port %04x, len:=%u',[address,io_len]));

  if Boolean(BX_PIT_DEBUG) then
    BX_INFO(Format('pit: write to port %04x := %02x',[address,value]));
...
{$ifend}
      ...
      BX_PANIC(Format('pit: unsupported io write to port %04x := %02x',[address,value]));
  end;
