...
begin
  if (io_len > 1) then
    BX_PANIC(Format('io read from port %04x, len:=%u',[address, io_len]));
  ...
  case address of
    $20:
      begin
        if (Self.s.master_pic.b.read_reg_select)<>0 then begin (* ISR *)
          BX_INFO(Format('read master ISR := %02x',[Self.s.master_pic.b.isr]));
      	Result:=Self.s.master_pic.b.isr;
        exit;
	    end
      else begin (* IRR *)
        BX_INFO(Format('read master IRR := %02x',[Self.s.master_pic.b.irr]));
    ...
    $21:
      begin
        BX_INFO(Format('read master IMR := %02x',[Self.s.master_pic.b.imr]));
    ...
    $A0:
      ...
      if (Self.s.slave_pic.b.read_reg_select)<>0 then begin (* ISR *)
        BX_INFO(Format('read slave ISR := %02x', [Self.s.slave_pic.b.isr]));
    	Result:=(Self.s.slave_pic.b.isr);
      exit;
  	end
      else begin (* IRR *)
        BX_INFO(Format('read slave IRR := %02x', [Self.s.slave_pic.b.irr]));
    ...
    $A1:
      begin
        BX_INFO(format('read slave IMR := %02x',[Self.s.slave_pic.b.imr]));
        Result:=(Self.s.slave_pic.b.imr);
        exit;
      end;
    end;

  BX_PANIC(Format('io read to address %04x',[address]));
...
{$ifend} // !BX_USE_PIC_SMF
  ...
  if (io_len > 1) then
    BX_PANIC(format('io write to port %04x, len:=%u',[address, io_len]));

    if (1=2) then
      BX_INFO(format('IO write to %04x := %02x',[address, value]));
  ...
  case address of
    $20:
      begin
        ...
        // BX_INFO(('pic:master: init command 1 found %02x', (unsigned) value));
            if BX_PIC_DEBUG = 1 then begin
              ...
              BX_INFO(Format('        requires 4 := %u',[(value and $01)]));
              BX_INFO(Format('        cascade mod_e: [0:=cascade,1:=single] %u',[((value  and $02) shr 1)]));
            end;
            ...
            if ( (value  and $02) = 1 ) then
              BX_PANIC(('master: ICW1: single mod_e not supported'));
            if ( (value  and $08) = 1 ) then begin
              BX_PANIC(('master: ICW1: level sensitive mod_e not supported'));
            end
            else begin
      ...
      if (Value in [$C0..$C7]) then
          begin
            BX_INFO(Format('IRQ lowest command $%x',[value]));
      ...
      if (Value in [$E0..$E7]) then
          ...
          begin
            BX_PANIC(Format('write to port 20h := %02x',[value]));
          end;
    	end; (* switch (value) *)
    ...
    $21:
            ...
            begin
              ...
              Self.s.master_pic.b.init.byte_expected := 3;
        	    if (BX_PIC_DEBUG)<>0 then begin
          		  BX_INFO(Format('master: init command 2 := %02x',[value]));
          		  BX_INFO(Format('        offset := INT %02x',[Self.s.master_pic.a.interrupt_offset]));
        	    end;
              exit;
            end;
      if Self.s.master_pic.b.init.byte_expected = 3 then
        begin
    	    if (BX_PIC_DEBUG)<>0 then
  		      BX_INFO(Format('master: init command 3 := %02x',[value]));
      ...
      (* normal operation *)
      if (BX_PIC_DEBUG)<>0 then
        BX_INFO(Format('setting master pic IMR to %02x',[value]));
    ...
    $A0:
      begin
        ...
        Self.s.slave_pic.b.rotate_on_autoeoi := 0;
        if ( (value  and $02) = 1 ) then
          BX_PANIC(('slave: ICW1: single mod_e not supported'));
        if ( (value  and $08) = 1 ) then begin
          BX_PANIC(('slave: ICW1: level sensitive mod_e not supported'));
	      end
      	else
        begin
    ...
    $A1:
      ...
      if (BX_PIC_DEBUG=1) then
        BX_INFO(Format('setting slave pic IMR to %02x',[value]));
...
begin
  ...
  if (Self.s.master_pic.b.special_mask)<>0 then begin
    ...
    if (isr)<>0 then begin // !!!!!!!
      ...
      if (max_irq > 7) then BX_PANIC(('error in service_master_pic()'));
      end
  else
...
begin
  BX_INFO(Format('s.master_pic.imr := %02x',[Self.s.master_pic.b.imr]));
  BX_INFO(Format('s.master_pic.isr := %02x',[Self.s.master_pic.b.isr]));
  BX_INFO(Format('s.master_pic.irr := %02x',[Self.s.master_pic.b.irr]));
  BX_INFO(Format('s.master_pic.irq := %02x',[Self.s.master_pic.b.irq]));
end;
end.
