...
procedure text_update(old_text: PChar; new_text: PChar;
  cursor_x: longword; cursor_y: longword; cursor_state: Bit16u; nrows: unsigned);
...
procedure DrawChar(hdc: HDC; c: char; xStart: integer;
  yStart: integer; cColor: byte; cs_start: integer; cs_end: integer);
...
uses VgaBitmap, service, keyboard, Math, cpu;
...
function MainWndProc(Wnd: HWND; Msg, wParam, lParam: longint): integer; stdcall;
var
  ps: PAINTSTRUCT;
  VideoDC, MemDC: HDC;
  OldObject: THandle;
begin
  case Msg of
    ...
    WM_KEYDOWN, WM_SYSKEYDOWN:
      ...
      enq_key_event(HiWord(lParam) and $01FF, BX_KEY_PRESSED);
    ...
    WM_KEYUP, WM_SYSKEYUP:
      begin
      EnterCriticalSection(KeyCS);
      enq_key_event(HiWord(lParam) and $01FF, BX_KEY_RELEASED);
      leavecriticalsection(KeyCs);
      Result := 0;
      exit;
      end;
...
procedure enq_mouse_event;

var
  current: PQueueEvent;
begin
  EnterCriticalSection(mouseCS);
  if ((ms_xdelta <> 0) or (ms_ydelta <> 0)) then
    begin
...
begin
  ...
  ms_lastx  := stretched_x div 2;
  ms_lasty  := stretched_y div 2;
  ms_savedx := ms_lastx;
  ms_savedy := ms_lasty;
end;
...
begin
  EnterCriticalSection(mouseCS);
  bx_state     := ((ifthen((windows_state and MK_LBUTTON) <> 0, 1, 0) +
    (ifthen((windows_state and MK_RBUTTON) <> 0, 2, 0))));
  old_bx_state := bx_state xor implied_state_change;
  if (old_bx_state <> mouse_button_state) then
    begin
  ...
  ms_lastx  := x;
  ms_lasty  := y;
  if (bx_state <> mouse_button_state) then
    begin
    EnterCriticalSection(keyCS);
    enq_mouse_event();
...
begin
  ...
  if boolean((prev_block_cursor_y * 80 + prev_block_cursor_x) < nchars) then
    begin
    cChar := PChar(integer(new_text) + (prev_block_cursor_y * 80 +
  ...
  while i < nchars * 2 do
    begin
    if boolean((old_text[i] <> new_text[i]) or (old_text[i + 1] <> new_text[i + 1])) then
      begin
  ...
  if boolean(((cursor_y * 80 + cursor_x) < nchars) and (cs_start <= cs_end)) then
    ...
    if boolean(yChar >= 16) then
      ...
      while i < 16 do
        begin
        Data[i * 2] := (bx_vgafont[byte(cChar) * 16 + i]);
...
begin
  ...
  for c := 0 to 255 do
    ...
    for i := 0 to 15 do
      begin
      Data[i * 2] := bx_vgafont[c * 16 + i];
      end;
    SetBitmapBits(vgafont[c], 32, @Data);
    end;
  ReleaseDC(MainWnd, HDC);
  hdc      := GetDC(MainWnd);
  MemoryBitmap := CreateCompatibleBitmap(hdc, 1024, 768);
